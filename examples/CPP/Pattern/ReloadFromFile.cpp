...
/**
@example Pattern/ReloadFromFile.cpp
Reload from file example of using 51Degrees device detection.

This example shows how to:

1. Only maintain a reference to a single EnginePattern instance and use the
reference to process data.

2. Use the EnginePattern->refreshData() function to reload the dataset from the same
location and with the same set of properties.
...
The EnginePattern->refresh() function requires an existing engine with the
...
namespace FiftyoneDegrees {
	namespace Examples {
		namespace Pattern {
			/**
			 * Pattern Reload From File Example.
			 */
			class ReloadFromFile : public ExampleBase {
			public:
				/**
				 * @copydoc ExampleBase::ExampleBase(string, DeviceDetection::Pattern::ConfigPattern*)
				...
				void run() {
					...
					ExampleBase::SharedState state(engine, userAgentFileName);
