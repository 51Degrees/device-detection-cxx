...
/**
@example Pattern/MatchForDeviceId.c
...
```
fiftyoneDegreesResultsPatternFromDeviceId(
	results,
	deviceId,
	sizeof(deviceId),
	exception);
...
void fiftyoneDegreesPatternMatchForDeviceId(
	...
	ResultsPattern *resultsUserAgents = ResultsPatternCreate(&manager, 1, 0);

	// Create a resultsUserAgents instance to store and process DeviceId.
	ResultsPattern *resultsDeviceId = ResultsPatternCreate(&manager, 1, 0);
	...
	ResultsPatternFree(resultsDeviceId);
	ResultsPatternFree(resultsUserAgents);
...
int main(int argc, char* argv[]) {
	...
	fiftyoneDegreesPatternMatchForDeviceId(
		dataFilePath,
		&config);
