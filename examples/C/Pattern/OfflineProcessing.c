...
/**
@example Pattern/OfflineProcessing.c
...
static char* getPropertyValueAsString(
	...
	EXCEPTION_CREATE;
	ResultsPatternGetValuesStringByRequiredPropertyIndex(
		results,
...
typedef struct t_offline_processing_state {
	...
	ResultsPattern *results; /**< The results used by the thread */
} offlineProcessState;
...
static void process(const char *userAgent, void *state) {
	...
	DataSetPattern *dataSet = (DataSetPattern*)offline->results->b.b.dataSet;
	ResultsPatternFromUserAgent(
		offline->results,
		userAgent,
		strlen(userAgent),
		exception);
	result = (ResultPattern*)offline->results->items;
	...
	for (i = 0; i < dataSet->b.b.available->count; i++) {
		ResultsPatternGetValues(
			offline->results,
			i,
			exception);
		if (offline->results->values.count == 0) {
...
void run(
	...
	state.results = ResultsPatternCreate(manager, 1, 0);
	dataSet = (DataSetPattern*)state.results->b.b.dataSet;
				
	printf("Starting Offline Processing Example.\n");
	
	// Print CSV headers to output file.
	fprintf(state.output, "\"User-Agent\",\"Method\",\"Difference\",\"Rank\"");
	for (i = 0; i < dataSet->b.b.available->count; i++) {
	...
	ResultsPatternFree(state.results);
}
