...
/**
@example Pattern/ReloadFromMemory.c
...
2. Use the #fiftyoneDegreesPatternReloadManagerFromMemory function to reload the
...
The #fiftyoneDegreesPatternReloadManagerFromMemory function requires an
...
/**
 * Returns a basic hash code for the string value provided.
 * @param value string whose hash code is required.
 * @return the hash code for the string provided.
 */
 ...
 */
static unsigned long getHashCode(ResultsPattern *results) {
	EXCEPTION_CREATE;
	...
	for (requiredPropertyIndex = 0;
		...
		valueName = STRING(ResultsPatternGetValues(
			results,
			requiredPropertyIndex,
			exception)->data.ptr);
...
static void executeTest(const char *userAgent, void *state) {
	...
	ResultsPattern *results = ResultsPatternCreate(thread->manager, 1, 0);
	...
	EvidenceAddString(
		...
		"User-Agent",
		userAgent);
	ResultsPatternFromEvidence(results, evidence, exception);
...
static void run(
	...
	if (ThreadingGetIsThreadSafe()) {
		...
		while (state.threadsFinished < THREAD_COUNT) {
			status = PatternReloadManagerFromMemory(
				manager,
				reader->startByte,
				reader->length,
				exception);
...
void fiftyoneDegreesPatternReloadFromMemoryRun(
	...
	// Set the required properties for patterning each test thread.
	...
	ResourceManager manager;
	status = PatternInitManagerFromMemory(
		&manager,
		&config,
		&reqProps,
		reader.startByte,
...
int main(int argc, char* argv[]) {
	...
	fiftyoneDegreesPatternReloadFromMemoryRun(
		dataFilePath,
		userAgentFilePath,
