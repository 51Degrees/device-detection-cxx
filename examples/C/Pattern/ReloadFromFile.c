...
/**
@example Pattern/ReloadFromFile.c
...
2. Use the #fiftyoneDegreesPatternReloadManagerFromOriginalFile function to
...
The #fiftyoneDegreesPatternReloadManagerFromOriginalFile function requires an
...
static void run(
	...
	if (ThreadingGetIsThreadSafe()) {
		...
		while (state.threadsFinished < THREAD_COUNT) {
			status = PatternReloadManagerFromOriginalFile(
				manager,
				exception);
			if (status == SUCCESS) {
				numberOfReloads++;
...
void fiftyoneDegreesPatternReloadFromFileRun(
...
int main(int argc, char* argv[]) {
	...
	fiftyoneDegreesPatternReloadFromFileRun(
		dataFilePath,
		userAgentFilePath,
