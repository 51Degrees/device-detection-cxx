...
/**
@example Pattern/StronglyTyped.c
...
valueName = FIFTYONE_DEGREES_STRING(
	fiftyoneDegreesResultsPatternGetValues(
		results,
		requiredPropertyIndex,
		exception)->data.ptr);
...
static bool getPropertyIsBool(
	...
	fiftyoneDegreesDataSetPattern *dataSet =
		(fiftyoneDegreesDataSetPattern*)results->b.b.dataSet;
	fiftyoneDegreesProperty *property = fiftyoneDegreesPropertyGetByName(
		dataSet->properties,
		dataSet->strings,
		propertyName,
		...
		exception);
	bool isBool =
		property->valueType == FIFTYONE_DEGREES_PROPERTY_VALUE_TYPE_BOOLEAN;
...
static bool getPropertyValueAsBool(
	...
	fiftyoneDegreesDataSetBase *dataSet =
		(fiftyoneDegreesDataSetBase*)results->b.b.dataSet;
...
static void printBoolValues(fiftyoneDegreesResultsPattern *results) {
	uint32_t i;
	const char *propertyName;
	fiftyoneDegreesDataSetBase *dataSet =
		(fiftyoneDegreesDataSetBase*)results->b.b.dataSet;
	for (i = 0; i < dataSet->available->count; i++) {
		propertyName = FIFTYONE_DEGREES_STRING(
			dataSet->available->items[i].name.data.ptr);
		if (getPropertyIsBool(results, propertyName)) {
			printf("   %s: %d\n",
				propertyName,
...
void fiftyoneDegreesPatternStronglyTyped(
	...
	printf("Starting Strongly Typed Example.\n");
...
int main(int argc, char* argv[]) {
	...
	fiftyoneDegreesConfigPattern config = fiftyoneDegreesPatternDefaultConfig;
	...
	fiftyoneDegreesPatternStronglyTyped(
		dataFilePath,
		&config);
