...
|Example C|Example CPP|Description|
|-------|-----------|--------|
|GettingStarted.c|GettingStarted.cpp|This example shows how to get set up with a Device Detection Cloud aspect engine and begin using it to process User-Agents and Client Hints.<br>Also showcase `query.51D_deviceId` high-priority evidence feature (streamlines profile selection).|
|MatchMetrics.c|MatchMetrics.cpp|This example shows how to view metrics associated with the results of processing with a Device Detection aspect engine using the Hash algorithm.|
||MetaData.cpp|This example shows how to interrogate the meta data associated with the contents of a Device Detection Hash data file.|
|MemHash.c||This example measures the memory usage of the Hash algorithm.|
|OfflineProcessing.c||This example shows how process data for later viewing using a Device Detection Hash data file.|
|Performance.c||Command line performance evaluation program which takes a file of user agents and returns a performance score measured in detections per second per CPU core.|
|ProcHash.c||Command line process which takes a user agent via stdin and return device properties via stdout.|
|ReloadFromFile.c|ReloadFromFile.cpp|This example illustrates how to reload the data file from the data file on disk without restarting the application.|
|ReloadFromMemory.c|ReloadFromMemory.cpp|This example illustrates how to reload the data file from a continuous memory space that the data file was read into without restarting the application.|
|StronglyTyped.c|StronglyTyped.cpp|This example  takes some common User-Agents and returns the value of the IsMobile property as a boolean.|
|MatchForDeviceId.c||Retrieve device by deviceId used as evidence. DeviceId may have been obtained previously and stored to later lookup the device properties.|
|FindProfiles.c||Find all profiles that match a certain property value - in this example we count the number of mobile (IsMobile=true) profiles|
