...
PropertyMetaDataCollectionForComponentPattern::PropertyMetaDataCollectionForComponentPattern(
			...
			if (property != nullptr) {
				...
				if (propertyComponent->componentId ==
					...
					properties.push_back(shared_ptr<PropertyMetaData>(
						PropertyMetaDataBuilderPattern::build(
							dataSet,
							property)));
				}
...
PropertyMetaData* PropertyMetaDataCollectionForComponentPattern::getByIndex(
	uint32_t index) {
	return new PropertyMetaData(*properties.at(index));
}

PropertyMetaData* PropertyMetaDataCollectionForComponentPattern::getByKey(
	string name) {
...
uint32_t PropertyMetaDataCollectionForComponentPattern::getSize() {
